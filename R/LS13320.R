#' Parse export files from LS13 320
#'
...
read_LS13320 <- function(pattern  = ".+\\.\\$ls\\.txt") {
  files <-
    dir(pattern)

  GSdata <-
    map(files, validateFile) %>%
    transpose() %>%
    map(bind_rows)
  return(GSdata)
}
...
extractParams <- function(fileName, startEndList) {
  ...
  paramstbl[1,] <-
    ...
    .[2,]
  paramstbl <- paramstbl %>%
    mutate_at(vars(UID), str_replace_all, fixed(".$ls"), "") %>%
    mutate(
      StartDepth = as.numeric(str_split(SampleID, "-")[[1]][1]),
      EndDepth = as.numeric(str_split(SampleID, "-")[[1]][2]),
